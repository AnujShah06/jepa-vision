...
[1.5d] **Best probe configuration (locked for multi-seed sweep): target encoder, mean pool, z-score, lr=3e-3, 200 epochs** — determined by probe_diag.py on seed-0, n=4000, val only. Exhaustive ablation recovered only +3.0 pp (0.571 → 0.601) over the baseline config; residual −9.9 pp gap to Gate 1B floor (≥0.70) is a representation quality issue. Target encoder ≈ context encoder (+0.001), suggesting EMA has not produced strongly diverged representations at 150 epochs — watch across seeds. Last-2-layer concat gave the best structural variant (+2.1 pp) but target+zscore was chosen as the canonical config for interpretability and simplicity.
